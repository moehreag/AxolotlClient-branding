...
public class FriendsScreen extends Screen {
	...
	protected void init() {
		...
		unblockButton = addDrawableSelectableElement(ButtonWidget.builder(Text.translatable("api.users.unblock"),
			b -> {
				b.active = false;
				FriendRequest.getInstance().unblockUser(widget.getSelectedOrNull().getUser()).thenRun(() -> client.execute(this::refresh));
			}).positionAndSize(this.width / 2 - 50, this.height - 28, 100, 20).build());
		cancelButton = addDrawableSelectableElement(ButtonWidget.builder(CommonTexts.CANCEL, b -> {
			b.active = false;
