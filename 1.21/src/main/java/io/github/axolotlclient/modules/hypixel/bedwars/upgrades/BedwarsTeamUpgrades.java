...
public class BedwarsTeamUpgrades {
	...
	public final TeamUpgrade forge = new TieredUpgrade(
		...
		new int[]{2, 4}, new int[]{4, 6}, (graphics, x, y, width, height, upgradeLevel) -> {
		if (upgradeLevel == 0) {
			graphics.drawTexture(Identifier.ofDefault("textures/block/furnace_front.png"), x, y, 0, 0, width, height, width, height);
		} else {
			if (upgradeLevel == 2) {
			...
			graphics.drawTexture(Identifier.ofDefault("textures/block/furnace_front_on.png"), x, y, 0, 0, width, height, width, height);
	...
	public final TeamUpgrade featherFalling = new TieredUpgrade("feather_falling", Pattern.compile("^\\b[A-Za-z0-9_§]{3,16}\\b purchased Cushioned Boots .{1,2}\\s*$"),
		...
		if (upgradeLevel == 1) {
			graphics.drawItem(new ItemStack(Items.IRON_BOOTS), x, y);
		} else {
			graphics.drawItem(new ItemStack(Items.DIAMOND_BOOTS), x, y);
		}
	});
