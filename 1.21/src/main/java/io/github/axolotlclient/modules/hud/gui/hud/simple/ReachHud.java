...
public class ReachHud extends SimpleTextHudEntry {
	...
	public static double getAttackDistance(Entity attacking, Entity receiving) {
		...
		maxPos = compareTo(camera, maxPos.add(0, 0, receiving.getBounds().maxZ), max);
		maxPos = compareTo(camera, maxPos.add(0, 0, receiving.getBounds().minZ), max);
		maxPos = compareTo(camera, maxPos.add(0, receiving.getBounds().maxY, 0), max);
		maxPos = compareTo(camera, maxPos.add(0, receiving.getBounds().minY, 0), max);
		maxPos = compareTo(camera, maxPos.add(receiving.getBounds().maxX, 0, 0), max);
		maxPos = compareTo(camera, maxPos.add(receiving.getBounds().minX, 0, 0), max);

		// Max reach distance that want to account for
		double d = max.get() + .5;
		Vec3d possibleHits = camera.add(rotation.x * d, rotation.y * d, rotation.z * d);
		Box box = attacking.getBounds().stretch(rotation.multiply(d)).expand(1.0, 1.0, 1.0);
