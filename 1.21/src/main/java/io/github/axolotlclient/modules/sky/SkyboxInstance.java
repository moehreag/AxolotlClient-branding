...
public abstract class SkyboxInstance {
	...
	protected final Identifier MOON_PHASES = Identifier.ofDefault("textures/environment/moon_phases.png");
	protected final Identifier SUN = Identifier.ofDefault("textures/environment/sun.png");
	...
	protected void setupRotate(MatrixStack matrices, float delta, float brightness) {
		...
		matrices.multiply(Axis.X_POSITIVE.rotationDegrees(rotationStatic[0]).get(dest));
		matrices.multiply(Axis.Y_POSITIVE.rotationDegrees(rotationStatic[1]).get(dest));
		matrices.multiply(Axis.Z_POSITIVE.rotationDegrees(rotationStatic[2]).get(dest));
		if (rotate) {
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, brightness);
			matrices.multiply(Axis.X_POSITIVE.rotationDegrees(rotationAxis[0]).get(dest));
			matrices.multiply(Axis.Y_POSITIVE.rotationDegrees(rotationAxis[1]).get(dest));
			matrices.multiply(Axis.Z_POSITIVE.rotationDegrees(rotationAxis[2]).get(dest));
			matrices.multiply(Axis.Y_POSITIVE.rotationDegrees(-90.0F).get(dest));
			matrices.multiply(Axis.X_NEGATIVE.rotationDegrees(
				MinecraftClient.getInstance().world.getSkyAngle(delta) * 360F * rotationSpeed).get(dest));
			matrices.multiply(Axis.Z_NEGATIVE.rotationDegrees(rotationAxis[0]).get(dest));
			matrices.multiply(Axis.Y_NEGATIVE.rotationDegrees(rotationAxis[1]).get(dest));
			matrices.multiply(Axis.X_NEGATIVE.rotationDegrees(rotationAxis[2]).get(dest));
		}
	}
	...
	protected void clearRotate(MatrixStack matrices) {
		Matrix4f dest = new Matrix4f();
		matrices.multiply(Axis.X_NEGATIVE.rotationDegrees(rotationStatic[0]).get(dest));
		matrices.multiply(Axis.Y_NEGATIVE.rotationDegrees(rotationStatic[1]).get(dest));
		matrices.multiply(Axis.Z_NEGATIVE.rotationDegrees(rotationStatic[2]).get(dest));
	}
	...
	protected void renderDecorations(MatrixStack matrices, Matrix4f projectionMatrix, float delta, Runnable runnable) {
		...
		matrices.multiply(Axis.Y_POSITIVE.rotationDegrees(-90.0F).get(dest));
		matrices.multiply(
			Axis.X_POSITIVE.rotationDegrees(MinecraftClient.getInstance().world.getSkyAngle(delta) * 360.0F).get(dest));
		...
		if (showSun) {
			...
			bufferBuilder.xyz(matrix4f2, -k, 100.0F, -k).uv0(0.0F, 0.0F);
			bufferBuilder.xyz(matrix4f2, k, 100.0F, -k).uv0(1.0F, 0.0F);
			bufferBuilder.xyz(matrix4f2, k, 100.0F, k).uv0(1.0F, 1.0F);
			bufferBuilder.xyz(matrix4f2, -k, 100.0F, k).uv0(0.0F, 1.0F);
		...
		if (showMoon) {
			...
			bufferBuilder = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
			bufferBuilder.xyz(matrix4f2, -k, -100.0F, k).uv0(p, q);
			bufferBuilder.xyz(matrix4f2, k, -100.0F, k).uv0(t, q);
			bufferBuilder.xyz(matrix4f2, k, -100.0F, -k).uv0(t, o);
			bufferBuilder.xyz(matrix4f2, -k, -100.0F, -k).uv0(p, o);
			BufferRenderer.drawWithShader(bufferBuilder.end());
		}
		if (showStars) {
