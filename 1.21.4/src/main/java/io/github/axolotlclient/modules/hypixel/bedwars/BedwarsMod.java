...
public class BedwarsMod implements AbstractHypixelMod {
	...
	public void onMessage(ReceiveChatMessageEvent event) {
		...
		if (currentGame != null) {
			...
			String time = "§7" + currentGame.getFormattedTime() + ChatFormatting.RESET + " ";
			if (!event.isCancelled() && showChatTime.get()) {
				// Add time to every message received in game
				if (event.getNewMessage() != null) {
					event.setNewMessage(Component.literal(time).append(event.getNewMessage()));
				} else {
					event.setNewMessage(Component.literal(time).append(event.getFormattedMessage()));
				}
			}
		} else if (enabled.get() && targetTick < 0 && BedwarsMessages.matched(GAME_START, rawMessage).isPresent()) {
			// Give time for Hypixel to sync
			targetTick = Minecraft.getInstance().gui.getGuiTicks() + 10;
		}
	}
	...
	public void tick() {
		if (currentGame != null) {
			...
			if (currentGame.isStarted()) {
				...
				currentGame.tick();
				Minecraft.getInstance().gui.getTabList().setHeader(null);
			} else {
				if (checkReady()) {
					currentGame.onStart();
				}
			...
			if (targetTick > 0 && Minecraft.getInstance().gui.getGuiTicks() > targetTick) {
	...
	private boolean checkReady() {
		for (PlayerInfo player : Minecraft.getInstance().player.connection.getListedOnlinePlayers()) {
			String name =
				Minecraft.getInstance().gui.getTabList().getNameForDisplay(player).getString().replaceAll("§.", "");
			if (name.charAt(1) == ' ') {
				return true;
			}
		}
	...
	public void onScoreboardRender(ScoreboardRenderEvent event) {
		...
		if (!ChatFormatting.stripFormatting(event.getObjective().getDisplayName().getString()).contains("BED WARS")) {
			return;
		}
		...
		List<PlayerScoreEntry> filteredScores =
			scores.stream().filter(p_apply_1_ -> !p_apply_1_.isHidden()).toList();
		waiting = filteredScores.stream().anyMatch(score -> {
			PlayerTeam team = scoreboard.getPlayerTeam(score.owner());
			String format = ChatFormatting.stripFormatting(PlayerTeam.formatNameForTeam(team, Component.literal(score.owner())).getString())
				.replaceAll("[^A-z0-9 .:]", "");
