...
public class SnappingHelper {
	...
	public void renderSnaps(GuiGraphics graphics) {
		Integer curx, cury;
		if ((curx = getRawXSnap()) != null) {
			DrawUtil.fillRect(graphics, new Rectangle(curx, 0, 1, window.getGuiScaledHeight()), LINE_COLOR);
		}
		if ((cury = getRawYSnap()) != null) {
			DrawUtil.fillRect(graphics, new Rectangle(0, cury, window.getGuiScaledWidth(), 1), LINE_COLOR);
		}
		//renderAll();
	}
	...
	public void renderAll(GuiGraphics graphics) {
		for (Integer xval : x) {
			DrawUtil.fillRect(graphics, new Rectangle(xval, 0, 1, window.getGuiScaledHeight()), ClientColors.WHITE);
		}
		for (Integer yval : y) {
			DrawUtil.fillRect(graphics, new Rectangle(0, yval, window.getGuiScaledWidth(), 1), ClientColors.WHITE);
		}
	}
