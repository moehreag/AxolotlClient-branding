...
public class AutoGG implements AbstractHypixelMod {
	...
	private void populateGGStrings() {
		...
		ggStrings.put("pvp.land", addToList("The match has ended!", "Match Results", "Winner:", "Loser:"));

		ggStrings.put("minemen.club", addToList("Match Results"));
	}

	private void populateGFStrings() {
		gfStrings.put("hypixel.net", addToList("SkyWars Experience (Kill)", "coins! (Final Kill)"));

		gfStrings.put("bedwarspractice.club", addToList(client.getUser().getName() + " FINAL KILL!"));

		gfStrings.put("pvp.land", addToList("slain by " + client.getUser().getName()));

		gfStrings.put("minemen.club", addToList("killed by " + client.getUser().getName() + "!"));
	}
	...
	public void onMessage(Component message) {
		if (client.getCurrentServer() != null) {
			serverMap.keySet().forEach(s -> {
				if (serverMap.get(s).get() && client.getCurrentServer().ip.contains(s)) {
					if (gf.get()) {
	...
	private void processChat(Component messageReceived, List<String> options, String messageToSend) {
