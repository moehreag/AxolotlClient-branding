	...
	private void renderBossBar(MatrixStack matrices, int x, int y, BossBar bossBar) {
		...
		if (bar.get()) {
			...
			if (bossBar.getOverlay() != BossBar.Style.PROGRESS) {
				DrawableHelper.drawTexture(matrices, x, y, 0, 80 + (bossBar.getOverlay().ordinal() - 1) * 5 * 2, 182, 5,
					256, 256);
			}
			...
			if (i > 0) {
				...
				if (bossBar.getOverlay() != BossBar.Style.PROGRESS) {
					DrawableHelper.drawTexture(matrices, x, y, 0, 80 + (bossBar.getOverlay().ordinal() - 1) * 5 * 2 + 5,
						i, 5, 256, 256);
				}
			}
		}
