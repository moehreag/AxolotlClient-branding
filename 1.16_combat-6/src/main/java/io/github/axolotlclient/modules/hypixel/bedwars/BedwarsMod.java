...
public class BedwarsMod implements AbstractHypixelMod {
	...
	private boolean checkReady() {
		for (PlayerListEntry player : MinecraftClient.getInstance().player.networkHandler.getPlayerList()) {
			String name = MinecraftClient.getInstance().inGameHud.getPlayerListWidget().getPlayerName(player).getString().replaceAll("§.", "");
			if (name.charAt(1) == ' ') {
				return true;
			}
		}
	...
	public void onScoreboardRender(ScoreboardRenderEvent event) {
		...
		waiting = filteredScores.stream().anyMatch(score -> {
			...
			String format = Formatting.strip(Team.modifyText(team, new LiteralText(score.getPlayerName())).getString()).replaceAll("[^A-z0-9 .:]", "");
