...
public class KeystrokeHud extends TextHudEntry {
	...
	private Keystroke deserializeKey(Map<String, ?> json) {
		if ("option".equals(json.get("type"))) {
			KeyBinding key = KeyBindAccessor.getAllKeyBinds().get((String) json.get("option"));
	...
	public enum SpecialKeystroke {
		SPACE(new Rectangle(0, 54, 53, 7), MinecraftClient.getInstance().options.keyJump, (hud, stroke, matrices) -> {
