...
public class PrivacyNoticeScreen extends Screen {
	...
	private void addButtons(int y) {
		addDrawableChild(new ButtonWidget(width / 2 - 50, y, 100, 20,
			Text.translatable("api.privacyNotice.accept"), buttonWidget -> {
			client.setScreen(parent);
			APIOptions.getInstance().privacyAccepted.set("accepted");
			accepted.accept(true);
		}));
		addDrawableChild(new ButtonWidget(width / 2 + 55, y, 100, 20,
			Text.translatable("api.privacyNotice.deny"), buttonWidget -> {
			client.setScreen(parent);
			APIOptions.getInstance().enabled.set(false);
			APIOptions.getInstance().privacyAccepted.set("denied");
			accepted.accept(false);
		}));
		addDrawableChild(new ButtonWidget(width / 2 - 155, y, 100, 20,
			Text.translatable("api.privacyNotice.openPolicy"), buttonWidget -> {
