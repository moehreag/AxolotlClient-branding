...
public class Util {
	...
	public static void sendChatMessage(String msg) {
		...
		Text text2 = net.minecraft.util.Util.map(preview.tryConsumeResponse(msg), ChatPreview.Response::response);
		if (msg.startsWith("/")) {
			MinecraftClient.getInstance().player.sendCommand(msg.substring(1), text2);
		} else {
			MinecraftClient.getInstance().player.m_ghcmhmee(msg, text2);
		}
	}
