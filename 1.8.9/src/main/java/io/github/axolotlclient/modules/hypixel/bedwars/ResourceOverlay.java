...
public class ResourceOverlay extends BoxHudEntry {
	...
	public void renderComponent(float delta) {
		draw(s -> ItemUtil.getTotal(client, s));
	}

	private void draw(Function<ItemStack, Integer> countFunction) {
		var pos = getPos();
		int x = pos.x() + 1;
		int y = pos.y() + 1;
		for (Item item : RESOURCES) {
			...
			if (amount > 0) {
				...
				ItemUtil.renderGuiItemOverlay(client.textRenderer, stack, x, y, String.valueOf(amount), -1, true);
				x += 18;
			}
		}
	}
	...
	public void renderPlaceholderComponent(float delta) {
		draw(s -> PLACEHOLDER.get(s.getItem()));
	}
